...
try:  # pragma: no cover - script execution fallback
    from tron_roll_call_hero.package_diagnostics import PROJECT_NAME, PROJECT_RELEASE_LABEL, PROJECT_VERSION, SPEC_NAME
    from tron_roll_call_hero.release_checklist import EXPECTED_WINDOWS_ZIP, validate_release_artifact
...
def build_release_build_preflight(
    ...
    *,
    ...
    commands = [
        ...
        "python -m tron_roll_call_hero.tron package-check --json",
        "python -m tron_roll_call_hero.tron release-check --json",
        ...
        "python -m tron_roll_call_hero.tron release-check --dist dist --json",
    ]
...
def run_release_build_pipeline(
    ...
    *,
    ...
    commands = [
        ...
        ("package_check", [sys.executable, "-m", "tron_roll_call_hero.tron", "package-check", "--json"], False, True),
        ("release_check", [sys.executable, "-m", "tron_roll_call_hero.tron", "release-check", "--json"], False, True),
        (
            "pyinstaller",
            [
                sys.executable,
    ...
    final_check = _run_command(
        [sys.executable, "-m", "tron_roll_call_hero.tron", "release-check", "--dist", str(dist_path), "--json"],
        cwd=base,
        base_dir=base,
