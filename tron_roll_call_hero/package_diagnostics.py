...
PROJECT_NAME = "tron-roll-call-hero"
...
def _runtime_package_modules(package_dir: Path) -> List[str]:
    ...
    for path in package_dir.glob("*.py"):
        if path.name in {"__init__.py", "tron.py"}:
            continue
        modules.append("tron_roll_call_hero.{}".format(path.stem))
...
def validate_pyinstaller_spec(spec_path: Path, *, package_dir: Path | None = None) -> Dict[str, Any]:
    ...
    hidden_imports = sorted(
        {
            item
            for item in strings
            if item in {"aiohttp", "aiohttp.web", "yaml"} or item.startswith("tron_roll_call_hero.")
        }
    )
    ...
    package_path = Path(package_dir) if package_dir is not None else spec.parent / "tron_roll_call_hero"
...
def _pyproject_report(path: Path) -> Dict[str, Any]:
    ...
    checks = [
        ...
        _check("console script trothu", scripts.get("trothu") == "tron_roll_call_hero.tron:main", "trothu entrypoint", severity="fail"),
        _check(
            "console script long name",
            scripts.get(PROJECT_NAME) == "tron_roll_call_hero.tron:main",
...
def _release_builder_report() -> Dict[str, Any]:
    available = _module_available("tron_roll_call_hero.release_builder") or _module_available("release_builder")
    checks = [
        _check("release-build runner", available, "tron_roll_call_hero.release_builder importable", severity="warn"),
    ]
    return {
        "available": available,
...
def build_package_diagnostic_report(base_dir: Path, *, config: Mapping[str, Any] | None = None) -> Dict[str, Any]:
    ...
    pyinstaller = validate_pyinstaller_spec(base / SPEC_NAME, package_dir=base / "tron_roll_call_hero")
