...
try:  # pragma: no cover - script execution fallback
    ...
    from tron_roll_call_hero.app_shell_dashboard import build_shell_dashboard_cards, build_shell_policy
    from tron_roll_call_hero.radar_map_assist import build_radar_map_assist, validate_radar_point
    from tron_roll_call_hero.release_checklist import build_release_build_plan, build_release_checklist
    from tron_roll_call_hero.webview_sync import WebViewSyncError, build_webview_cookie_preview, build_webview_sync_status, parse_webview_cookie_export
...
<title>tron_roll_call_hero Companion Shell</title>
<style>
...
<body>
<header><h1>tron_roll_call_hero Companion Shell</h1></header>
<div class="layout">
...
def create_app_shell(
    ...
    *,
    ...
    async def action_handoff(request: Any) -> Any:
        ...
        templates = {
            "qr_submit": "python -m tron_roll_call_hero.tron qr paste --yes <QR_URL_OR_PAYLOAD>",
            "qr_fanout": "python -m tron_roll_call_hero.tron qr paste --all --yes <QR_URL_OR_PAYLOAD>",
            "webview_import": "python -m tron_roll_call_hero.tron webview import --input cookies.json --profile default --save --json",
            "reauth": "python -m tron_roll_call_hero.tron bot serve --adapter generic",
            "release_check": "python -m tron_roll_call_hero.tron release-check --json",
        }
        template = templates.get(action) or "python -m tron_roll_call_hero.tron status --json"
        return _json(
            {
                "status": "ok",
                "handoff": {
