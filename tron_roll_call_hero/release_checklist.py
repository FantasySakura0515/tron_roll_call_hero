...
ARTIFACT_NAME_RE = re.compile(r"^(tron-roll-call-hero|THU_Auto_Rollcall)-v?[\w.\-]+", re.IGNORECASE)
...
def build_release_checklist(base_dir: Path, *, config: Mapping[str, Any] | None = None, dist_dir: Path | None = None) -> Dict[str, Any]:
    ...
    release_builder_available = importlib.util.find_spec("tron_roll_call_hero.release_builder") is not None or importlib.util.find_spec("release_builder") is not None
    checks = [
